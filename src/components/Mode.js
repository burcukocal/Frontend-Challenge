...
const Mode = ({ darkMode, setDarkMode, language, setLanguage}) => {
    ...
    return (
        <div className="modes">
            ...
            {language === "en" ? 
            (
                <p className="font-bold text-sm inline-block text-[#6B7280]"> {`${darkMode ? data.en.lightMode : data.en.darkMode} `} </p>) 
            : (
                <p className="font-bold text-sm inline-block text-[#6B7280]"> {`${darkMode ? data.tr.lightMode : data.tr.darkMode}`} </p>
            )
            }
            </div>
            <div className="line">
